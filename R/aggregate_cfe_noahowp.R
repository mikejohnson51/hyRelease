...
#' @param nwm_dir the NWM geo directory
...
aggregate_cfe = function(gpkg,
  ...
  x2 = map_vars(r =  soils[[grepl(paste0(soil_gm_var, collapse = "|"), names(soils))]],
                w = nwm_w_1000m,
                file = NULL,
                precision,
                single_layer = single_layer,
  ...
  x3 = map_vars(r =  soils[[grepl(paste0(soil_mean_var, collapse = "|"), names(soils))]],
                w = nwm_w_1000m,
                file = NULL,
                precision,
                single_layer = single_layer,
  ...
  ### WRF
  wrf = rast(file.path(nwm_dir, 'wrfinput_CONUS.nc'), subds = c("IVGTYP", "ISLTYP"))
  ...
  if(!is.null(flowline_name)){
    ...
    crosswalk = select(crosswalk, ID, member_COMID) %>%
...
aggregate_noahowp = function(gpkg,
  ...
  soils = suppressWarnings(rast(file.path(nwm_dir, 'soilproperties_CONUS_FullRouting.nc')))
  terra::ext(soils) = template$ext
  terra::crs(soils) = template$crs
  ...
  x1 = map_vars(r =  soils[[grepl(paste(soil_mean_var, collapse = "|"), names(soils))]],
                w = nwm_w_1000m,
                file = NULL,
                precision,
                single_layer = single_layer,
